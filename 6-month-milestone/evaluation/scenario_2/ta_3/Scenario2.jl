...
# ╔═╡ 12285bf3-17b3-49bd-a57a-25a08f2c4fa0
begin
...
# ╔═╡ 4d75fc47-8b06-48c1-b741-e71da1c21ec7
using ModelingToolkit: @set!

# ╔═╡ b9341834-a2e3-11ed-0f55-fb50ef228629
md"""
...
# ╔═╡ f4492b0f-da68-4f93-9458-0eaf3599ccaa
begin sidarthe = read_json_acset(LabelledPetriNet, "sidarthe.json")
sys = ODESystem(sidarthe) end

# ╔═╡ 9c6765aa-0b25-4d52-9a39-0b8b0a8a712d
begin 
...
# ╔═╡ 462e80a1-fa1d-4113-b505-266e5d076fce
md"""
...
# ╔═╡ 064c578d-6df6-4eac-93ab-8dff1a253a62
md"""
### Unit Tests
...
# ╔═╡ be627117-9573-4f4d-a15e-79914ae20e0e
begin
...
# ╔═╡ 72a985c4-54a9-4798-89d7-5bd82e9e8871
md"""
#### Unit Test 2
...
# ╔═╡ dde7d6cc-1b3b-4682-a148-f1160328657e
begin
...
# ╔═╡ 88d20589-249f-445f-92a1-e116866f0010
let
	solne = solve(probne, Tsit5())
	p = plot(solne, vars = idart)
end

# ╔═╡ c386a87c-5d68-4d57-97bc-81254e0ae27a
plot(solne, idxs = [sum(idart)], lab = "total infected")

# ╔═╡ b5571c9d-223e-4534-92c0-701df57b9a0b
plot(solt1; idxs = Diagnosed)

# ╔═╡ b7358b5d-96a1-44ce-82fe-b2081f62b0ce
plot(solt1; idxs = idart)

# ╔═╡ 9bd0fe98-4768-43f3-bc92-14769aad9ece
@test solt1[Infected + Healed] == solt1[Infected] + solt1[Healed]

# ╔═╡ d9e87a78-c8ab-409e-a13f-570348653fa1
plot(solt1.t, solt1[sum(idart)] * ITALY_POPULATION; label = "IDART absolute")

# ╔═╡ bf75eaf9-e9c8-411c-8a54-28188dd61fd0
plot(solt1.t, solt1[sum(idart)]; label = "IDART percent")

# ╔═╡ ccb151c4-44c4-4a42-821f-166dd6923dda
xmax, xmaxval = get_max_t(prob_test1, sum(idart))

# ╔═╡ 6a4f0fe0-3b8c-441e-9c98-0d9fe6bd54e0
@test isapprox(xmax, 47; atol = 4)

# ╔═╡ 479c2f04-a308-4922-be7b-147a1e045807
md"""
This test passes with SBML.jl
"""

# ╔═╡ 4b9a8a43-5ada-496d-baec-e61a1e2b5154
@test_broken isapprox(xmaxval, 0.002; atol = 0.01)

# ╔═╡ 59ce4d7c-0d4c-4f09-92a8-7d300c7a5c26
md"""
...
# ╔═╡ fc038e7f-bb13-41e1-a537-315feb34f5a2
begin pbounds = [param => [
...
# ╔═╡ c394c911-8eca-419c-929f-eb6e0378e87e
begin threshold_observable = (Infected + Diagnosed + Ailing + Recognized + Threatened) /
                       sum(states(sys))
cost = (epsilon + theta)
...
# ╔═╡ 65192d40-3ff3-4bc9-8890-98813765f5a5
begin 
import Graphviz_jll
sidarthe_v = read_json_acset(LabelledPetriNet, "sidarthe_v.json")

mca_sidarthe_v = mca(sidarthe, sidarthe_v)
AlgebraicPetri.Graph(mca_sidarthe_v[1]) end

# ╔═╡ 8785fcf3-45d9-4a75-8565-6abf67585d3c
begin
...
# ╔═╡ 7cea77d4-dd97-45aa-b2bb-664b97d1fad3
md"""
...
# ╔═╡ c6bbe7ff-3914-47fb-99d4-cbd0b4570292
sysv, tau1, tau2, defs_v2, u0valsv, phi = let
	...
	sysv = complete(sysv)
	sysv, tau1, tau2, defs_v2, u0valsv, phi
end

# ╔═╡ 540f89b0-076c-4a59-923d-6b9b0300eb9c
begin
...
# ╔═╡ 7eb2b2ed-9fd7-4047-995b-9f731f3c6746
plot(solv, idxs = [og_states; sysv.V])

# ╔═╡ c4b8a6b1-679a-4719-8fd4-bfeedacccd9c
plot(solt1; idxs = sum(idart))

# ╔═╡ 12541f99-d9b2-4baa-874c-c9f70f9fa6a9
xmax1, xmaxval1 = get_max_t(probv, sum(idart) * ITALY_POPULATION)

# ╔═╡ 1f5eb327-8a62-404f-9907-9083a8a62b8a
xmax2, xmaxval2 = get_max_t(probv, sum(idart))

# ╔═╡ 872edcd9-b575-4c33-97c2-e020373fe842
@test isapprox(xmax2, 47; atol = 1)

# ╔═╡ 31ea3b85-b64c-4c7b-a92c-d0ee31f62ddf
@test isapprox(xmaxval2, 0.6; atol = 0.1)

# ╔═╡ 89521891-8ed6-489b-ae92-ca2671e8f3c6
md"""
...
# ╔═╡ 00ac9fe7-b053-43da-8e39-03a99b19a536
begin
	threshold_observable2 = (Infected + Diagnosed + Ailing + Recognized + Threatened) /
                       sum(states(sysv))
	plot(solv, idxs = [threshold_observable2], lab = "total infected")
	hline!([1 / 3], lab = "limit")
end

# ╔═╡ e68f56c9-4204-4775-827d-f5b1d8f93003
md"""
### b.ii
...
# ╔═╡ 410310d9-e924-45f8-ad17-228877508708
plt, solv3, probv3, R_t, R0, cumulative_inf = let
	...
	solv3 = solve(probv3)
	p = plot(solv3)
	p, solv3, probv3, R_t, R0, cumulative_inf
end;

# ╔═╡ 97d2d553-609d-4d30-af8c-13610b2c254f
plt

# ╔═╡ 42a4726b-2448-4deb-a8a6-95891227a027
solv3

# ╔═╡ 908f331a-6298-4fc7-92e6-f131ad333963
md"""
...
# ╔═╡ ac49d74f-0531-4bd7-ae79-c332277b89d4
begin
...
# ╔═╡ a295c6f5-ae79-435b-ba76-d5a828ada61c
md"""
We can see that increasing the rate of vaccination is the most effective.
"""

# ╔═╡ c5209037-64e3-47bd-931f-c6490491829d
plot(map(opt_results) do opt_result
...
# ╔═╡ 06e4e9b8-d212-406c-885c-01ab31e1b59f
begin
		base = "https://raw.githubusercontent.com/ChrisRackauckas/ASKEM_Evaluation_Staging/main/docs/src/Scenario2/"
		files = [
			"Giordano2020.xml",
			"sidarthe.json",
			"sidarthe_v.json"
...
git-tree-sha1 = "3036a6246db65c6e4837997ba3b4a52e9a023f64"
...
git-tree-sha1 = "d94e9902cf38131d596a7644b8aa128cf172d9a1"
...
git-tree-sha1 = "1e0a1fde551d043af93996a52430c762b32d30c4"
...
git-tree-sha1 = "3411269b1073aea4ce12b958d5c3f8c55134cf62"
...
git-tree-sha1 = "af5254c7c05b4c82364f9d70dd48f4ff89f683b3"
...
git-tree-sha1 = "fb5ad0700d214344d3034e5b2f09b63684b37fc9"
...
git-tree-sha1 = "b488fc28dfae4c8ec3d61a34a0143a4245e7b13b"
...
git-tree-sha1 = "b1cb83e39576e0f0c31413159fea0fe3020f75c1"
...
deps = ["MathOptInterface", "Optimization", "Reexport", "SparseArrays"]
git-tree-sha1 = "8493b32aa07712eb50c5e71468c0f383281849f2"
...
git-tree-sha1 = "f1f22e8a603a188ca76ae39a5f6caf560bd3d024"
...
git-tree-sha1 = "3b96c84832be31fce33de760a2cd5f4fdc050c23"
