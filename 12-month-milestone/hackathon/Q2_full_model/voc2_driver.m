...
new_ai      = 0.8;
t_new_voc   = 974; %daysact('1-jan-2020','1-sep-2022');
...
[t,y] = ode15s(@(t,y) svair(t,y,beta,beta_v1,beta_v2,beta_R,ai_beta_ratio,gamma,nu_v1,nu_v2,nu_R,ai,ai_V,ai_R,mu,mu_I,mu_IV,new_beta,new_beta_v1, ...
    new_beta_v2,new_beta_R,new_ai,t_new_voc), tvector, y0, odeopts);
S   = y(:,1);
SVR = y(:,2);
V1PF= y(:,3); V1AZ=y(:,4);
V2PF= y(:,5); V2AZ=y(:,6);
...
%     dAAdt(nt)   = ai(2)*(+ beta(2).*S(nt).*(IA(nt)+IAV(nt)+IAR(nt)+ai_beta_ratio(1)*(AA(nt)+AAR(nt))) + beta(2).*SVR(nt).*(IA(nt)+IAV(nt)+IAR(nt)+ai_beta_ratio(2)*(AA(nt)+AAR(nt))));
